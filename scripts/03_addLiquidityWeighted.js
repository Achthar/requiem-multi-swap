...
async function main() {
    ...
    // deploy the pool
    const poolContract = new ethers.Contract(addresses.weighted[chainId].pools[0], new ethers.utils.Interface(WeihghtedPool.abi), operator)

    console.log("Pool", poolContract.address)

    const multis = await poolContract.getTokenMultipliers()
    const rawAmounts = [10, 1, 30000]
