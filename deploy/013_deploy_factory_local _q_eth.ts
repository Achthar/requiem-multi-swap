...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
	...
	await execute('RequiemQRouterBackup', { from: localhost, value: BigNumber.from(10000) }, 'onSwapExactETHForTokens',
		qSwap2,
		BigNumber.from('0'), //out min
		localhost,// address to,
		deadline,// uint256 deadline
	...
	await execute('RequiemQRouterBackup', { from: localhost }, 'onSwapExactTokensForETH',
		qSwap21,
		BigNumber.from(1000),
		BigNumber.from('0'), //out min
		localhost,// address to,
	...
	const qSwap13 = [
		{
			structure: 0,
			pool: pairWethUsdc, // address pool;
			tokenIn: usdc.address,
			tokenOut: weth.address,
	...
	const routerContract = await ethers.getContractAt('RequiemQRouterBackup', router.address);
	const amnts = await routerContract.onSwapTokensForExactTokens(qSwap23,
	...
	await execute('RequiemQRouterBackup', { from: localhost }, 'onSwapTokensForExactETH',
		qSwap3,
