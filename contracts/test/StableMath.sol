...
library StableMath {
        ...
        **********************************************************************************************/
        ...
        for (uint256 i = 0; i < 255; i++) {
            ...
            for (uint256 j = 0; j < numTokens; j++) {
                ...
                D_P = BalancerMath.divDown(BalancerMath.mul(D_P, invariant), BalancerMath.mul(balances[j], numTokens));
            }

            prevInvariant = invariant;

            invariant = BalancerMath.divDown(
                BalancerMath.mul(
                    // (ampTimesTotal * sum) / AMP_PRECISION + D_P * numTokens
                    (BalancerMath.divDown(BalancerMath.mul(ampTimesTotal, sum), _AMP_PRECISION).add(BalancerMath.mul(D_P, numTokens))),
                    invariant
                ),
                // ((ampTimesTotal - _AMP_PRECISION) * invariant) / _AMP_PRECISION + (numTokens + 1) * D_P
                (
                    BalancerMath.divDown(BalancerMath.mul((ampTimesTotal - _AMP_PRECISION), invariant), _AMP_PRECISION).add(
                        BalancerMath.mul((numTokens + 1), D_P)
                    )
                )
            );
    ...
    */
    ...
    function _getTokenBalanceGivenInvariantAndAllOtherBalances(
        ...
        revert("Errors.STABLE_GET_BALANCE_DIDNT_CONVERGE");
    }
