...
contract RequiemZap is ReentrancyGuard {
    ...
    IRequiemWeightedPairFactory public requiemWeightedPairFactory;
    ...
    constructor(IUniswapV2Router _uniRouter, IRequiemRouter _requiemRouter) {
        ...
        requiemWeightedPairFactory = IRequiemWeightedPairFactory(_requiemRouter.factory());
    ...
    function zapInToken(
        ...
        if (_from == IRequiemWeightedPair(_to).token0() || _from == IRequiemWeightedPair(_to).token1()) {
            ...
            lpAmt = _pairDeposit(_to, _from, other, amounts[0].sub(sellAmount), otherAmount, msg.sender, requiemWeightedPairFactory.isPair(_to), transferResidual);
        } else {
    ...
    function zapOutWithPermit(
        ...
        // permit
        IRequiemWeightedPair(_from).permit(msg.sender, address(this), _approvalAmount, _deadline, v, r, s);
    ...
    function calculateSwapInAmount(
        ...
        (uint32 tokenWeight0, uint32 tokenWeight1, ) = requiemWeightedPairFactory.getWeightsAndSwapFee(pair);
    ...
    function _getRemovedReserveAmountOut(
        ...
        (, uint256 reserveIn, uint256 reserveOut, uint32 tokenWeightIn, uint32 tokenWeightOut, uint32 swapFee) = requiemFormula.getFactoryReserveAndWeights(
    ...
    function _swapETHToLp(
        ...
        if (pair.token0() == WETH || pair.token1() == WETH) {
            ...
            {
                (uint32 tokenWeight0, , ) = requiemWeightedPairFactory.getWeightsAndSwapFee(lp);
