    ...
    describe("zap token", () => {
        ...
        describe("zap out", () => {
            ...
            it("zap out with path", async () => {
                await addLiquidityUniPair(toWei(100), toWei(100));
                await zap.setRequiemPairs(token0.address, token2.address, [pair.address, pairUni.address]);
                await zap.setRequiemPairs(token1.address, token2.address, [pairUni.address]);
    ...
    describe("zap eth", () => {
        ...
        describe("zap in", () => {
            ...
            it("zap in with path", async () => {
                await addLiquidityUniETHPair(toWei(10), toWei(100));
                await zap.setRequiemPairs(WETHPartner.address, WETH.address, [WETHPair.address]);
            ...
            it("zap in with path return dust", async () => {
                await token2.approve(zap.address, maxUint256);
                await addLiquidityUniETHPair(toWei(10), toWei(100));
                await zap.setRequiemPairs(token2.address, WETH.address, [pairUniETH.address]);
        ...
        describe("zap out", () => {
            ...
            it("zap out with path", async () => {
                await addLiquidityUniETHPair(toWei(10), toWei(100));
                await zap.setRequiemPairs(WETH.address, token2.address, [pairUniETH.address]);
                await zap.setRequiemPairs(WETHPartner.address, token2.address, [WETHPair.address, pairUniETH.address]);
