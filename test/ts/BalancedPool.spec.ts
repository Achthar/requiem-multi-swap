...
import { approveAll, balancerMathFixture, distributeTokens, ERC20Fixture, pairFixture, BalancedPoolFixture, balancedPoolFixture, swapRouterFixture, SwapRouterFixture, thiefRouterFixture, tokenFixture, validatePoolBals, bnAbs } from "./shared/fixtures";
import {
    maxUint256,
    mineBlockTimeStamp,
    toWei
...
describe("Balanced Pools", () => {
    ...
    let fixture: BalancedPoolFixture
    ...
    beforeEach(async () => {
        ...
        fixture = await balancedPoolFixture(wallet, [tokens.token0, tokens.token1, tokens.token2], fee, flashFee, withdrawFee);
    ...
    it("Allows liquidity removal with no fees exact LP in", async () => {
        ...
        const testFixture = await balancedPoolFixture(wallet, [tokens.token0, tokens.token1, tokens.token2], fee, flashFee, zero);
        await approveAll(other, tokens, testFixture.pool.address)
        await approveAll(wallet, tokens, testFixture.pool.address)
    ...
    it('FlashSwap: valid, insufficient fee and reentrant', async () => {
        ...
        await fixture.pool.onFlashSwapGivenIn(flashSwapRecipient.address, tokens.token0.address, tokens.token1.address, testAmount, wallet.address, '0x')

        await flashSwapRecipient.setRepay(false)

        await expect(
            fixture.pool.onFlashSwapGivenIn(flashSwapRecipient.address, tokens.token0.address, tokens.token1.address, testAmount, flashSwapRecipient.address, '0x')
        ).to.be.revertedWith("insufficient in")
        ...
        await fixture.pool.onFlashSwapGivenOut(flashSwapRecipient.address, tokens.token0.address, tokens.token1.address, testAmount, flashSwapRecipient.address, '0x')

        await flashSwapRecipient.setRepay(false)

        await expect(
            fixture.pool.onFlashSwapGivenOut(flashSwapRecipient.address, tokens.token0.address, tokens.token1.address, testAmount, flashSwapRecipient.address, '0x')
        ).to.be.revertedWith("insufficient in")
    ...
    it('FlashSwap: gas cost', async () => {
        ...
        txIn = await fixture.pool.onFlashSwapGivenIn(repayFlashSwap.address, tokens.token0.address, tokens.token1.address, testAmount, repayFlashSwap.address, '0x')
        ...
        txIn = await fixture.pool.onFlashSwapGivenOut(repayFlashSwap.address, tokens.token0.address, tokens.token1.address, testAmount, repayFlashSwap.address, '0x')
